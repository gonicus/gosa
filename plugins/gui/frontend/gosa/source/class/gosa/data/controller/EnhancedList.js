...
/**
 * Augmented {qx.data.controller.List} to allow grouping and sorting via delegate.
 * To allow grouping of list elements, the used ListElements (given by delegate.createItem) must
 * have a 'listItemType' property. This controller changes that property to 'group' or 'item' depending on
 * the shown content.
 */
qx.Class.define("gosa.data.controller.EnhancedList", {
  ...
  members : {
     ...
     * of the binding.
     *
     * @param config {Map} Map holding index and type of the item
     */
    __addItem : function(config) {
     ...
     *   ListItem.
     * @param config {Map} The index and type of the ListItem.
     */
    _bindListItem : function(item, config) {
      var delegate = this.getDelegate();
      // if a delegate for creating the binding is given, use it
      item.setListItemType && item.setListItemType(config.type);
      switch (config.type) {
        case "group":
        ...
        default:
          if (delegate != null && delegate.bindItem != null) {
            delegate.bindItem(this, item, config.index);
            // otherwise, try to bind the listItem by default
          }
          else {
            this.bindDefaultProperties(item, config.index);
          }
          break;
      }
    },
    ...
    _removeBindingsFrom : function(item) {
      ...
      for (var i = 0; i < this.__boundPropertiesReverse.length; i++) {
        ...
        if (id != null) {
          ...
          item.setUserData(this.__boundPropertiesReverse[i] + "ReverseBindingId", null);
        }
      }
      ;
    },
    ...
    __buildUpLookupTable : function() {
      ...
      if (delegate != null) {
        ...
        if (delegate.filter) {
          filteredModel = model.filter(delegate.filter);
        }
        if (delegate.sorter) {
          filteredModel.sort(delegate.sorter);
        }
        var lastGroup = null;
        filteredModel.forEach(function(item) {
          ...
          if (delegate.group) {
            var groupName = delegate.group(item);
            if (groupName !== lastGroup) {
              this.__groupLookupTable.push(groupName);
              this.__lookupTable.push({ index: this.__groupLookupTable.length-1, type: 'group'});
            }
            lastGroup = groupName;
          }
          this.__lookupTable.push(entry);
        }, this);
      } else {
        this.__lookupTable = model.map(function(entry, index) {
          return {index: index, type: 'item' };
        }, this).toArray();
      }
    },
