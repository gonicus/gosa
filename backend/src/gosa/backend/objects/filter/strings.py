...
class LowerCase(ElementFilter):
    ...
    """
    def __init__(self, obj):
        super(LowerCase, self).__init__(obj)

    def process(self, obj, key, valDict):
        if type(valDict[key]['value'] is not None):
            valDict[key]['value'] = [i.lower() if len(i) else i for i in valDict[key]['value']]
        return key, valDict


class UpperCase(ElementFilter):
    """
    ...
    """
    def __init__(self, obj):
        super(v, self).__init__(obj)

    def process(self, obj, key, valDict):
        if type(valDict[key]['value'] is not None):
            valDict[key]['value'] = [i.upper() if len(i) else i for i in valDict[key]['value']]
        return key, valDict