    ...
    try:
        if not proxy.login(sys_id, key):
            ...
            if mode == "start":
                ...
                # send config to dbus
                dbus_proxy = PluginRegistry.getInstance("DBUSProxy")
                if "menu" in config:
                    # send to client
                    print("sending generated menu for user %s" % user)
                    dbus_proxy.callDBusMethod("configureUserMenu", user, dumps(config["menu"]))

                if "printer-setup" in config and "printers" in config["printer-setup"]:
                    dbus_proxy.callDBusMethod("deleteAllPrinters")
                    for p_conf in config["printer-setup"]["printers"]:
                        dbus_proxy.callDBusMethod("addPrinter", p_conf)

                    if "defaultPrinter" in config["printer-setup"] and config["printer-setup"]["defaultPrinter"] is not None:
                        dbus_proxy.callDBusMethod("defaultPrinter", config["printer-setup"]["defaultPrinter"])
                ...
                if "resolution" in config and config["resolution"] is not None and len(config["resolution"]):
                    ...
                    dbus_proxy.callDBusMethod("configureUserScreen", user, config["resolution"][0], config["resolution"][1])
